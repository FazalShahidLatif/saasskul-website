...
export default function SignupForm() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    // Has session = email confirmation disabled → redirect immediately
    if (data.session) {
        ...
        <div className="space-y-1.5">
          ...
          {password.length > 0 && (
            <div className="flex gap-1 mt-1.5">
              {[1,2,3].map(l => (
                <div key={l} className={`h-1 flex-1 rounded-full transition-all ${strength >= l ? l===1?'bg-red-400':l===2?'bg-yellow-400':'bg-brand-400' : 'bg-gray-100 dark:bg-white/8'}`} />
              ))}
            </div>
          )}
        </div>
