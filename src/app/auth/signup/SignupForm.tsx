...
export default function SignupForm() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (data.session) {
      router.push(plan ? `/pricing?plan=${plan}` : '/onboarding')
    } else {
      setSuccess(true)
    }
  }
    ...
    return (
      <div className="text-center py-6 space-y-4">
        ...
        <p className="text-xs text-gray-500">
          Didn&apos;t receive it?{' '}
          <button onClick={() => setSuccess(false)} className="text-brand-500 hover:underline">Try again</button>.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-gray-200 dark:border-white/8 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-white/5 disabled:opacity-60 transition-all"
      >
        {oauthLoading ? (
